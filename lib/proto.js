...
exports.attr = function(path){
  return this.accessor.accessor(path);
};
...
exports.set = function(name, val, broadcaster){
  ...
  this.data[name] = attr
    ? attr.typecast(val, this)
    : val;

  if (this.data[name] !== prev) {
...
function findAttr(content, name) {
  while (content) {
    ...
    if (undefined !== content.data[name]) return content.data[name];

    // try getting default value
    var attr = content.constructor.attrs[name];
    if (attr && attr.hasDefaultValue)
      return content.data[name] = attr.apply(content);
